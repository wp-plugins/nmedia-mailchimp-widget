...
class NMMailChimp extends NM_Framwork_V2_nm_mailchimp{
	...
	function _setup(){
		...
		if( $this->get_option('_mc_api_key') != '')
			$this -> mc =  new NM_Mailchimp( trim($this->get_option('_mc_api_key')) );
		...
		*/
		...
		$this -> ajax_callbacks = array('none'			=> true,
										...
										'mc_add_group'	=> false,
										'mc_save_list_val'	=> false,
										'woo_mc_save_list_val'	=> false,
										'mc_del_group'	=> false,
										...
										'form_designer'	=> false);		// Subscription Form Designer
	...
	function mc_save_list_val(){
		if (isset($_REQUEST['list_val'])) {
			$list_val = $_REQUEST['list_val'];
			update_option( 'nm_mailchimp_list_val', $list_val);
		}
		die(0);
	}

	function woo_mc_save_list_val(){
		if (isset($_REQUEST['list_val'])) {
			$list_val = $_REQUEST['list_val'];
			update_option( 'nm_woo_mailchimp_list_val', $list_val);
		}
		die(0);
	}
	...
	function form_designer(){
		if (isset($_REQUEST['formData'])) {
			$newData = $_REQUEST['formData'];
			update_option( 'nm_mailchimp_form_settings', $newData);
		}
	}
	...
	function subscribe_user(){
		...
		$intrests	= (isset($_REQUEST['intrests']) ? $_REQUEST['intrests'] : '');
		...
		if($intrests){
			$groupings = '';
			foreach($intrests as $intrest_id => $groups){
				$groupings['id'] = $intrest_id;
				$groups_name = array();
				foreach($groups as $groupd_id => $group_name){
					$groups_name[] = $group_name;
				}
				$groupings['groups'] = $groups_name;
				
				$intrests_vars[] = $groupings;
			}
			
		}
		
    if($intrests_vars){
			$vars['groupings']	= $intrests_vars;
		}
		...
		// create user if admin set it to yes
		if($this -> get_option('_create_user') == 'yes'){
			...
			if ( $userid && !is_wp_error( $userid ) && $this -> get_option('_user_detail') == 'yes') {

				$suject = __('New user registration ', $this->plugin_meta ['shortname']) . $email;
				...
				$msg .= '<p><a href="' . wp_login_url() . '" title="Login">'.__('Click here to login', $this->plugin_meta ['shortname']).'</a>';
				//$msg = str_replace("\n", "<br />", $msg);
				
				
				wp_mail( $email, $suject, $msg, $this->get_mail_header() );
