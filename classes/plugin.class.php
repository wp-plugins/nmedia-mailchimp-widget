...
class NMMailChimp extends NM_Framwork_V2_nm_mailchimp{
	...
	function subscribe_user(){
		...
		$email		= $_REQUEST['vars']['EMAIL'];
		$listid		= $_REQUEST['listid'];
		$groupings	= json_decode(stripslashes($_REQUEST['grouping']), true);
		...
		$resp = '';
		//var_dump($this->mc->apikey);
		$resp = $this -> mc -> lists -> subscribe($listid, array('email' => htmlentities($email)), $vars);
		var_dump($resp);
		//echo json_encode($resp);
